...
function initRenderer() {
  ...
  window.addEventListener('route:renderer', (e) => {
    // Check if it's the specific layout loading route
    const alignMatch = e.detail.hash ? e.detail.hash.match(/\?align=([^&]+)/) : null;
    const vcamMatch = e.detail.hash ? e.detail.hash.match(/\?vcam=([^&]+)/) : null;
    if (alignMatch) {
